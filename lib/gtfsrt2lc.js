...
class Gtfsrt2lc {
    constructor(path, routes, trips, calendar_dates, uris) {
        this._path = path;
        this._routes = routes;
        this._trips = trips;
        this._calendar_dates = calendar_dates;
        this._uris = uris;
    }
    ...
    async parse(format) {
        ...
        if(format === 'jsonld') {
            ...
            return readable.pipe(new Connections2Triples()).pipe(new N3.StreamWriter({ prefixes : {
    ...
    resolveValue(param, tripId, connParams) {
        ...
        switch (source) {
            ...
            case 'calendar_dates':
                trip = this.trips.get(tripId);
                caldate = this.calendar_dates.get(trip.service_id);
                value = caldate[attr];
                break;
            case 'connection':
