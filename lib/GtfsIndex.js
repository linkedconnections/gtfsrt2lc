...
class GtfsIndex {
    ...
    extractIndexes(resolve, reject) {
        ...
        let calendar_dates_index = new Map();
        ...
        let trips = fs.createReadStream('.tmp/trips.txt', { encoding: 'utf8', objectMode: true })
            ...
            .pipe(csv({ objectMode: true, headers: true }));
        let calendar_dates = fs.createReadStream('.tmp/calendar_dates.txt', { encoding: 'utf8', objectMode: true })
            .on('error', async err => {
                await this.cleanUp();
                reject(err);
            })
        ...
        calendar_dates.pipe(through2.obj(async (cd, enc, done) => {
            if (cd['service_id']) {
                calendar_dates_index.set(cd['service_id'], cd);
            }
            done();
        }))
            .on('error', async err => {
        ...
        let finish = async () => {
            ...
            if (count === 3) {
                ...
                await this.cleanUp();
                resolve([routes_index, trips_index, calendar_dates_index]);
            }
        };
    }
