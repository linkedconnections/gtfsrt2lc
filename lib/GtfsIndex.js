...
class GtfsIndex {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (this.path.startsWith('http') || this.path.startsWith('https')) {
                ...
                if (!fs.existsSync(this.path)) {
                    ...
                    fs.createReadStream(this.path).pipe(unzip.Extract({ path: this.auxPath }))
                        .on('error', async err => {
                            await this.cleanUp();
                            reject(err);
                        })
    ...
    handleResponse(res, resolve, reject) {
        ...
        res.pipe(unzip.Extract({ path: this.path }))
            .on('error', async err => {
                await this.cleanUp();
                reject(err);
            })
    ...
    extractIndexes(resolve, reject) {
        ...
        fs.createReadStream(this.auxPath + '/routes.txt', { encoding: 'utf8', objectMode: true })
        ...
        fs.createReadStream(this.auxPath + '/trips.txt', { encoding: 'utf8', objectMode: true })
            .pipe(csv.parse({ objectMode: true, headers: true }))
            .on('data', trip => {
                if (trip['trip_id']) {
        ...
        fs.createReadStream(this.auxPath + '/stops.txt', { encoding: 'utf8', objectMode: true })
            .pipe(csv.parse({ objectMode: true, headers: true }))
            .on('data', stop => {
                if (stop['stop_id']) {
        ...
        fs.createReadStream(this.auxPath + '/stop_times.txt', { encoding: 'utf8', objectMode: true })
            .pipe(csv.parse({ objectMode: true, headers: true }))
            .on('data', stop_time => {
