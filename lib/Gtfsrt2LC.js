...
class Gtfsrt2LC {
    ...
    async parse2Json(headers) {
        let rawData = await this.getRawData(headers);
        let data = gtfsrt.FeedMessage.decode(rawData);
        this.jsonData = data;
        return data;
    }

    async getUpdatedTrips(headers) {
        let trips = [];
        if (!this.jsonData) {
            await this.parse2Json(headers);
        }
        ...
        try {
            ...
            Promise.all(this.jsonData.entity.map(async entity => {
                if (entity.tripUpdate) {
                    ...
                    if (!(await this.trips.get(tripId)) || !(await this.stop_times.get(tripId)) || (await this.stop_times.get(tripId)).length < 2) {
                        return;
                    }
                    ...
                    if (tripUpdate.trip.startTime) {
                        ...
                        tripStartTime = this.addDuration(serviceDay, this.parseGTFSDuration((await this.stop_times.get(tripId))[0]['departure_time']));
                    }
                    ...
                    let stopTimesSchedule = await this.stop_times.get(tripId);
                    ...
                    //console.error(completedUpdates);
                        ...
                        for (let [j, cu] of completedUpdates.entries()) {
                            ...
                            if(j < completedUpdates.length - 1) {
                                try {
                                    ...
                                    let pickupType = this.resolveScheduleRelationship(cu['scheduleRelationship'], stopTimesSchedule[pdIndex]['pickup_type']);
                                    ...
                                    let linked_connection = {
                                        ...
                                        "direction": (await this.trips.get(tripId))['trip_headsign'],
                                        "trip": tripURI,
                                        "route": routeURI,
    ...
    async resolveValue(param, stopId, tripId, tripStartTime, connection, resolve) {
        ...
        let trips = tripId ? await this.trips.get(tripId) : null;
        let routes = tripId ? await this.routes.get(trips['route_id']) : null;
        let stops = stopId ? await this.stops.get(stopId) : null;
