...
class Gtfsrt2LC {
    ...
    async parse(format, asObject) {
        ...
        try {
            ...
            //fs.writeFileSync('/home/julian/Desktop/rt-raw', rawData);
            //fs.writeFileSync('/home/julian/Desktop/rt-decoded.json', JSON.stringify(feed), 'utf8');
    ...
    completeCancelledUpdates(staticStops, serviceDay, now) {
        let cancelledUpdates = [];

        for (let i in staticStops) {
            let departure = this.addDuration(serviceDay, this.parseGTFSDuration(staticStops[i]['departure_time']));

            if (now <= departure) {
                cancelledUpdates.push({
                    'stopId': staticStops[i]['stop_id'],
                    'arrival': {
                        'delay': 0,
                        'time': this.addDuration(serviceDay, this.parseGTFSDuration(staticStops[i]['arrival_time'])).getTime() / 1000
                    },
                    'departure': {
                        'delay': 0,
                        'time': departure.getTime() / 1000
                    }
                });
            }
        }
        ...
        try {
            ...
            for (let i in staticStops) {
                ...
                if (staticStop === liveStop) {
                    ...
                    let verifiedUpdate = this.checkUpdate(liveStops[liveIndex], completedUpdates[completedUpdates.length - 1], staticStops[i], i, serviceDay);
                    ...
                    } else if (liveIndex === 0 && i == staticIndex - 1 && liveStops[liveIndex]['arrival']) {
    ...
    checkUpdate(update, prevUpdate, staticData, staticIndex, serviceDay) {
        try {
            // If the stop update is missing departure info and is not the last stop of the trip 
            // add it manually taking into account the arrival delay at this stop 
            if (!update['departure'] && staticIndex != staticData.length - 1) {
                update['departure'] = {
            ...
            if (!update['arrival'] && staticIndex != 0 && prevUpdate) {
                ...
                let newArrTime = (this.addDuration(serviceDay, this.parseGTFSDuration(staticData['arrival_time'])).getTime() / 1000) + prevUpdate['departure']['delay'];

                if (newArrTime <= depTime) {
                    update['arrival'] = {
                        'delay': prevUpdate['departure']['delay'],
                        'time': newArrTime
                    }
                } else {
