...
class Gtfsrt2LC {
    ...
    async deduceTripId(routeId, startTime, startDate, directionId) {
        if (this.tripsByRoute && this.firstStops && this.calendar && this.calendarDates) {
            ...
            await Promise.all(this.tripsByRoute.get(routeId).map(async t => {
            ...
            let today = new Date(startDate.substring(0, 4), startDate.substring(4, 6), startDate.substring(6, 8));
            for (let i = 0; i < trips.length; i++) {
                const service = this.calendar.get(trips[i]['service_id']);
                const exceptions = this.calendarDates.get(trips[i]['service_id']) || {};
                const minDate = new Date(service['start_date'].substring(0, 4), service['start_date'].substring(4, 6), service['start_date'].substring(6, 8));
                const maxDate = new Date(service['end_date'].substring(0, 4), service['end_date'].substring(4, 6), service['end_date'].substring(6, 8));
    ...
    checkUpdate(update, prevUpdate, staticData, staticIndex, staticLength, serviceDay) {
        try {
            ...
            if (update['departure'] && typeof update['departure'].toJSON === 'function' && !update['departure'].toJSON()['delay']) {
            ...
            if (update['arrival'] && typeof update['arrival'].toJSON === 'function' && !update['arrival'].toJSON()['delay']) {
