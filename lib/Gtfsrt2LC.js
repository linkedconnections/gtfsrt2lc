...
class Gtfsrt2LC {
   ...
   async parse(options) {
      ...
      if (options.format === 'jsonld') {
         ...
         return readable.pipe(new Connections2Triples(this.uris)).pipe(new N3.StreamWriter({ format: 'N-Triples' }));
      } else if (options.format === 'turtle') {
         return readable.pipe(new Connections2Triples(this.uris)).pipe(new N3.StreamWriter({
            prefixes: {
